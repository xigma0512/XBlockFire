...
function canDefuseBomb(bombEntity: Entity, player: Player) {
    ...
    if (distance > DEFUSE_RANGE) {
        system.run(() => player.onScreenDisplay.setActionBar(`${ColorTable[ColorType.Red]}There is no c4 in the range.`));
        return false;
    }
...
function defuseComplete(roomId: number, defuser: Player) {
    ...
    Broadcast.message(`${ColorTable[ColorType.Yellow]}Bomb has been defused by ${defuser.name}.`);
}
