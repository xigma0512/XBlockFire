...
function canDefuseBomb(bombEntity: Entity, player: Player) {
    ...
    if (distance > DEFUSE_RANGE) {
        system.run(() => player.onScreenDisplay.setActionBar(`${FormatCodeTable[FormatCodeEnum.Red]}There is no c4 in the range.`));
        return false;
    }
...
function defuseComplete(roomId: number, defuser: Player) {
    ...
    Broadcast.message(`${FormatCodeTable[FormatCodeEnum.Yellow]}Bomb has been defused by ${defuser.name}.`);
}
