...
export class BP_RoundEndPhase implements IPhaseHandler {
    ...
    on_entry() {
        ...
        for (const player of member.getPlayers()) {
            ...
            player.sendMessage(`${FormatCodeTable[FormatCodeEnum.Gray]}Round Income: +${earn}`);
        }
    ...
    on_running() {
        ...
        const actionbarText = `${FormatCodeTable[FormatCodeEnum.Yellow]}Next round start in ${(this.currentTick / 20).toFixed(0)} seconds.`;
