...
export class BP_IdlePhase implements IPhaseHandler {
    ...
    on_running() {
        ...
        let actionbarText = `${FormatCodeTable[FormatCodeEnum.Yellow]}Waiting for more players...`;

        if (AUTO_START && playerAmount >= AUTO_START_MIN_PLAYER) {
            actionbarText = `${FormatCodeTable[FormatCodeEnum.Green]}Game will start in ${(this.currentTick / 20).toFixed(0)} seconds.`;
            this._currentTick --;
        }

        if (this.currentTick !== COUNTDOWN_TIME && playerAmount < AUTO_START_MIN_PLAYER) {
            this._currentTick = COUNTDOWN_TIME;
            Broadcast.message(`${FormatCodeTable[FormatCodeEnum.Red]}Not enough players. Waiting for more players.`, members);
        }
