...
export class BP_ActionPhase implements IPhaseHandler {
    ...
    on_running() {
        ...
        const actionbarText = `${FormatCodeTable[FormatCodeEnum.Blue]}Round time: ${(this.currentTick / 20).toFixed(0)}`;
    ...
    private transitions() {
        ...
        if (endReason) {
            ...
            const separator = `${FormatCodeTable[FormatCodeEnum.White]}---------------\n`;
            ...
            switch (endReason) {
                case EndReasonEnum['Time-up']:
                    message.push(`${FormatCodeTable[FormatCodeEnum.Yellow]}Time Up. This Round Is Over.\n`);
                    message.push(`${FormatCodeTable[FormatCodeEnum.Yellow]}Blue Team Win\n`);
                ...
                case EndReasonEnum['Defender-Eliminated']:
                    message.push(`${FormatCodeTable[FormatCodeEnum.Yellow]}Red Team Win\n`);
