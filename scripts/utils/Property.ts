...
import { GrenadeTypeEnum } from '../modules/combat/weapon/WeaponEnum';
import { GunReloadStateEnum } from '../modules/combat/weapon/WeaponTypes';
...
function set_entity_dynamic_property<T extends EntityDynamicPropertyId>(
    entity: Entity | ItemStack,
    name: T,
    value: EntityDynamicPropertyList[T]
) {
...
function scoreboard_property<T extends ScoreBoardPropertyId>(entity: Entity, name: T) {
    ...
    try {
        return scoreboard!.getScore(entity) as ScoreBoardPropertyList[T];
    } catch {
        return 0;
    }
}

function set_scoreboard_property<T extends ScoreBoardPropertyId>(
    entity: Entity,
    name: T,
    value: ScoreBoardPropertyList[T]
) {
    if (world.scoreboard.getObjective(name) === undefined) world.scoreboard.addObjective(name);
...
function set_entity_native_property<T extends EntityNativePropertyId>(
    entity: Entity,
    name: T,
    value: EntityNativePropertyList[T]
) {
